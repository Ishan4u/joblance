...
<h2 style="text-align:center; margin-bottom:25px;">Requested Candidates</h2>
      <div class="container">
        <div class="row">
            <!-- fetch request candidate-details -->
            <?php
            $select_canditae = $conn->prepare("SELECT u_p.*,u.*, p.*, u.image AS u_image FROM user_post u_p INNER JOIN users u ON u_p.u_id = u.id INNER JOIN post p ON u_p.job_id = p.job_id WHERE p.com_id = ? ");
            $select_canditae->execute([$com_id]);
